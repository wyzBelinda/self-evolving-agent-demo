...
self_evolving_mcp = MCPToolset(
    connection_params=StdioConnectionParams(
        server_params=StdioServerParameters(
            command='python',
            args=['my_mcp_server/self_evolving_mcp.py'],    
            name='self_evolving_mcp',
        ),
        timeout=50,
    ),
)
...
root_agent = Agent(
    ...
    instruction=(
        "You are a helpful agent who can answer user questions about what you want to search.\
            You can use the 'self_evolving_mcp' tool to improve your search results.\
            When you use the 'self_evolving_mcp' tool, you can provide a user comment to improve the search results."
    ),
    # tools=[get_weather, get_current_time, search_papers, extract_info],
    tools=[tavily_mcp, bilibili_mcp, self_evolving_mcp],
)